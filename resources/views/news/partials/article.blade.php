	...
	<footer class="post-info">
		<div class="left">
			@if (Auth::check() AND Auth::user()->hasPermission('news_edit'))
				<a href="{{ action('Admin\NewsController@getEdit', $article->id) }}">Edit</a> |
				<a class="delete-news" href="{{ action('Admin\NewsController@postDelete', $article->id) }}" data-title="{{ $article->title }}">Delete</a>
			@endif
		</div>
		<div class="right">
			@if ($article->editor)
