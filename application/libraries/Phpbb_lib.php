...
class Phpbb_lib
{
	...
	public function __construct()
	{
		...
		// Load the phpBB database
		$this->phpbb_db = $this->CI->load->database('phpbb', TRUE);
	}
	...
	public function getUserById($userId)
	{
		...
		$this->phpbb_db->select('*');
		$this->phpbb_db->from($table_prefix . 'users');
		$this->phpbb_db->where('user_id', $userId);
		$this->phpbb_db->limit(1);

		$query = $this->phpbb_db->get();
	...
	public function getUserByName($username)
	{
		...
		$this->phpbb_db->select('*');
		$this->phpbb_db->from($table_prefix . 'users');
		$this->phpbb_db->where('username', $username);
		$this->phpbb_db->limit(1);
	...
	public function getUserGroupMembership()
	{
		...
		$this->phpbb_db->select('g.group_name');
		$this->phpbb_db->from($table_prefix . 'groups g');
		$this->phpbb_db->from($table_prefix . 'user_group u');
		$this->phpbb_db->where('u.user_id', $userId);
		$this->phpbb_db->where('u.group_id', 'g.group_id', FALSE);
